...
We can now e.g., perform 11&bsp;runs each with two different optimization algorithms&nbsp;$\algorithmStyle{A}$ and&nbsp;$\algorithmStyle{B}$ on one problem instance and compute the arithmetic mean of a performance measures for each set of runs.
...
Therefore, this means that one of the two algorithms is better &ndash; with a certain probability.
We could get the results we get either because $\algorithmStyle{A}$ is really better than $\algorithmStyle{B}$ or by pure coincidence, as artifact from the randomness of our algorithms.
...
Strictly speaking, if derived from an experiment, the statement "$\algorithmStyle{A}$ is better than $\algorithmStyle{B}$" makes only sense if we can give an upper bound&nbsp;$\alpha$ for the error probability.
...
\rel.figure{coin_toss}{The results of our coin tossing game, where I win 128&nbsp;times (red) and you only 32&nbsp;times (green).}{coin_toss.svgz}{width=58%} 
...
What you can do is use the opposite hypothesis&nbsp;$H_0$:
I did not cheat, the coin is fair and both of us have winning probability&nbsp;$q=0.5$.
...
If you reject&nbsp;$H_0$ and accept&nbsp;$H_1$, the $p$&nbsp;would be your probability to be wrong.
...
This is, roughly, how statistical tests work (at least those relevant in our context).
...
If that would have been the case, the data samples&nbsp;$A$ and&nbsp;$B$ would stem from the same algorithm.
They would be observations of the same random variable, i.e., elements from the same population.
...
We conduct a small experiment and measured the objective values of the final results in a few runs of the algorithms.
We get the two data sets&nbsp;$A$ and&nbsp;$B$, respectively:
...
Algorithm&nbsp;$\algorithmStyle{B}$ seems to produce the smaller objective values.
...
Let us therefore assume as null hypothesis&nbsp;$H_0$ that the observed difference did just happen by chance.
We assume that $\algorithmStyle{A} \equiv \algorithmStyle{B}$.
This would mean that the data samples&nbsp;$A$ and&nbsp;$B$ stem from the *same* algorithm (as $\algorithmStyle{A} \equiv \algorithmStyle{B}$).
The division into the two sets&nbsp;$A$ and&nbsp;$B$ would only be artificial, an artifact of our experimental design.
...
Since we only did one experiment, of course we encountered only one of these divisions.
...
Let's say we draw 4&nbsp;elements from&nbsp;$O$ to form a potential set&nbsp;$B'$.
This leaves the remaining 6&nbsp;elements for the potential set&nbsp;$A'$, meaning $\binom{10}{6}=210$ as well.
