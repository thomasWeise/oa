...
%
Another typical optimization task arises in \indexi{manufacturing}, namely the assignment (\inQuotes{\indexi{scheduling}}) of tasks (\inQuotes{jobs}) to machines in order to optimize a given performance criterion (\inQuotes{\indexi{objective}}).
...
In the \emph{\indexi{Job Shop Scheduling Problem}}~(\indexi{JSSP})~\cite{CGLL1995STAIA,GLLRK1979OAAIDSASAS,LLRKS1993SASAAC,L1982RRITTOMS,T1993BFBSP,BDP1996TJSSPCANST}, we have a factory (\inQuotes{shop}) with several machines.
...
Often, we are looking for the starting times that lead to the earliest completion of all jobs, i.e., the shortest \indexi{makespan}.
