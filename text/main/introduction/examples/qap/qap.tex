...
\begin{figure}%
\centering%
\includegraphics[width=0.85\linewidth]{\currentDir/qap_example}%
...
This scenario is called \pgls{optQAP}~\cite{BCPP1998TQAP}.
It has been subject to research since the 1950s~\cite{BK1957APATLOEA}.
\glspl{optQAP} appear in wide variety of scenarios such as the location of facilities on a plot of land or the placement of work stations on the factory floor.
Even if we need to place components on a circuit board in a way that minimizes the total wire length, we basically have a \pgls{optQAP}, too~\cite{S1961TBWPAPA}!
Despite being relatively simple to understand, the \pgls{optQAP} is hard to solve~\cite{SGA1976PCAP}.
%
\endhsection%
%
