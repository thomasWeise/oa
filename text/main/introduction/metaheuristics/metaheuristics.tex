...
The number of involved variables here could be the number of cities in a \pgls{optTSP}, the number of jobs or machines in a \pgls{optJSSP}, or the number of objects to pack in a, well, packing problem.
A big group of such complicated problems are called \npHard~\cite{LLRKS1993SASAAC,CPW1998AROMSCAAA}.
...
In \cref{fig:runtime_quality_tradeoff} we illustrate this idea on the example of the \glsFull{optTSP}~\cite{ABCC2006TTSPACS,LLRKS1985TTSPAGTOCO,GP2002TTSPAIV} briefly mentioned in \cref{sec:intro:logistics}.
The goal of solving the \pgls{optTSP} is to find the shortest round trip tour through $n$~cities.
The \pgls{optTSP} is \npHard~\cite{GJ1979CAIAGTTTONC,GP2002TTSPAIV}.
Today, it is possible to solve many large instances of this problem to optimality by using sophisticated \emph{exact} algorithms~\cite{CEG2007CWDPIFTSPT,C2013WT}.
Yet, finding the \emph{shortest possible tour} for a particular \pgls{optTSP} might still take many years if you are unlucky.
...
Before, I stated that great exact algorithms for the \pgls{optTSP} exist~\cite{CEG2007CWDPIFTSPT,C2013WT}, that can solve many TSPs quickly (although not all).
There are years and years of research in these algorithms.
Even the top heuristic and metaheuristic algorithm for the \pgls{optTSP} today result from many years of targeted research~\cite{H2009GKOSFTLKTH,NK2013APGAUEACFTTSP,W2016BNMDPCADIM} and their implementation from the algorithm specification alone can take months~\cite{WWLC2019IIIOADTM}.
Unfortunately, if you do not have plain \pgls{optTSP}, but one with some additional constraints --- say, time windows to visit certain cities --- the optimized, state-of-the-art \pgls{optTSP} solvers are no longer applicable.
